...
async def on_avr_connection_error(device_id: str, message):
...
async def handle_avr_address_change(avr_id: str, address: str) -> None:
    ...
    if device and device.address != address:
        _LOG.info("Updating IP address of configured LG soundbar %s: %s -> %s", avr_id, device.address, address)
...
async def on_avr_update(device_id: str, update: dict[str, Any] | None) -> None:
    ...
    for configured_entity in _get_entities(device_id):
        if isinstance(configured_entity, media_player.LGMediaPlayer):
...
def _register_available_entities(config_device: config.ConfigDevice, device: LGDevice) -> None:
    """
    ...
    entities = [
        media_player.LGMediaPlayer(config_device, device),
...
async def _async_remove(device: LGDevice) -> None:
...
async def main():
    ...
    logging.getLogger("lglib").setLevel(level)
